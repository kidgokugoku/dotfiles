...
local function apply_crop(meta)
    ...
    -- Apply crop.
    mp.command(string.format("%s set file-local-options/video-crop %sx%s+%s+%s", command_prefix, meta.w, meta.h, meta.x,
        meta.y))
end
...
local function detect_crop()
    ...
    if hwdec_current:find("-copy$") == nil and hwdec_current ~= "no" and hwdec_current ~= "crystalhd" and hwdec_current ~=
        "rkmpp" then
    ...
    mp.command(string.format('%s vf pre @%s:cropdetect=limit=%s:round=%d:reset=0', command_prefix, cropdetect_label,
        limit, round))
