...
local EXTENSIONS_VIDEO_DEFAULT = Set {'3g2', '3gp', 'avi', 'flv', 'm2ts', 'm4v', 'mj2', 'mkv', 'mov', 'mp4', 'mpeg',
                                      'mpg', 'ogv', 'rmvb', 'webm', 'wmv', 'y4m'}

local EXTENSIONS_AUDIO_DEFAULT = Set {'aiff', 'ape', 'au', 'flac', 'm4a', 'mka', 'mp3', 'oga', 'ogg', 'ogm', 'opus',
                                      'wav', 'wma'}

local EXTENSIONS_IMAGES_DEFAULT = Set {}
-- local EXTENSIONS_IMAGES_DEFAULT = Set {'avif', 'bmp', 'gif', 'j2k', 'jp2', 'jpeg', 'jpg', 'jxl', 'png', 'svg', 'tga',
--    'tif', 'tiff', 'webp'}
...
options.read_options(o, nil, function(list)
    ...
    if list.videos or list.additional_video_exts or list.audio or list.additional_audio_exts or list.images or
        list.additional_image_exts then
        create_extensions()
    end
...
local function scan_dir(path, current_file, dir_mode, separator, dir_depth, total_files, extensions)
    ...
    if dir_mode == "recursive" then
        for _, dir in ipairs(dirs) do
            scan_dir(prefix .. dir .. separator, current_file, dir_mode, separator, dir_depth + 1, total_files,
                extensions)
        end
    else
...
local function find_and_add_entries()
    ...
    local files = {}
    scan_dir(autoloaded_dir, path, o.directory_mode or mp.get_property("directory-mode", "lazy"),
        mp.get_property_native("platform") == "windows" and "\\" or "/", 0, files, extensions)
