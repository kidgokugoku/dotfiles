...
local function build_track_title(track, prefix, filename)
    ...
    if track['demux-samplerate'] then
        h(string.format('%.5g kHz', track['demux-samplerate'] / 1000))
    end
    if track['demux-bitrate'] then
        h(string.format('%.5g kbps', track['demux-bitrate'] / 1000))
    end
    if #hints > 0 then
            ...
            if track.selected and track.id == pos then
                ...
                if type == 'sub' then
                    if (prop == 'sid' and not get('sub-visibility')) or
                        (prop == 'secondary-sid' and not get('secondary-sub-visibility')) then
...
local function update_profiles_menu(menu)
    ...
    for _, profile in ipairs(profile_list) do
        if not (profile.name == 'default' or profile.name:find('gui') or profile.name == 'encoding' or profile.name ==
            'libmpv') then
            append_menu(submenu, {
                title = profile.name,
