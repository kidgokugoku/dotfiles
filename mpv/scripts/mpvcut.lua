...
local options = {
    ...
    save_directory = "C:/home/screenshot", -- required for web videos
...
local function init()
    ...
    if full_path then
        full_path_save = mp.command_native({"expand-path",
                                            options.save_directory .. "/" .. mp.get_property("media-title")})
...
ACTIONS.COPY = function(d)
    ...
    local args = {"ffmpeg", "-nostdin", "-y", "-loglevel", "error", "-ss", d.start_time, "-t", d.duration, "-i",
                  d.inpath, "-c", "copy", "-map", "0", "-dn", "-avoid_negative_ts", "make_zero", result_path}
    print("Saving clip...")
...
ACTIONS.COMPRESS = function(d)
    ...
    local args = {"ffmpeg", "-nostdin", "-y", "-loglevel", "error", "-ss", d.start_time, "-t", d.duration, "-i",
                  d.inpath, "-pix_fmt", "yuv420p", "-c:v", "libx264", "-b:v", target_bitrate .. "k", "-c:a", "copy",
                  result_path}
    ...
    if video_height then
        if video_height > options.resolution then
            ...
            args = {"ffmpeg", "-nostdin", "-y", "-loglevel", "error", "-ss", d.start_time, "-t", d.duration, "-i",
                    d.inpath, "-vf", res_line, "-pix_fmt", "yuv420p", "-c:v", "libx264", "-b:v", target_bitrate .. "k",
                    "-c:a", "copy", result_path}
        end
    end
...
ACTIONS.ENCODE = function(d)
    ...
    local args = {"ffmpeg", "-nostdin", "-y", "-loglevel", "error", "-ss", d.start_time, "-t", d.duration, "-i",
                  d.inpath, "-pix_fmt", "yuv420p", "-crf", "16", "-preset", "superfast", result_path}
    print("Saving clip...")
    mp.command_native_async({
        name = "subprocess",
        args = args,
