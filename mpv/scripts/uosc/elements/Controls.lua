...
function Controls:init_options()
    ...
    local shorthands = {
        ['sc'] = 'command:screenshot_monitor:keypress s?截屏',
        ['hwdec'] = 'cycle:computer:hwdec:no=memory/auto-safe=computer?解码',
        ['crop'] = 'command:crop:script-binding autocrop/toggle_crop?裁剪黑边',
        ['total-time'] = 'command:more_time:script-binding total_playtime/total_time?总时长',
        ['delete-file-next'] = 'command:delete:script-binding uosc/delete-file-next?删除并下一个',
    ...
    for i, item in ipairs(items) do
        ...
        config = shorthands[config_tooltip[1]] and split(shorthands[config_tooltip[1]], ' *%? *')[1] or
                     config_tooltip[1]
        ...
        elseif kind == 'gap' then
            table_assign(control, {
                sizing = 'gap',
                scale = 1,
                ratio = params[1] or 0.3,
                ratio_min = 0
            })
        elseif kind == 'command' then
            if #params ~= 2 then
                mp.error(string.format('command button needs 2 parameters, %d received: %s', #params,
                    table.concat(params, '/')))
            else
        ...
        elseif kind == 'cycle' then
            if #params ~= 3 then
                mp.error(string.format('cycle button needs 3 parameters, %d received: %s', #params,
                    table.concat(params, '/')))
            else
        ...
        elseif kind == 'button' then
            if #params ~= 1 then
                mp.error(string.format('managed button needs 1 parameter, %d received: %s', #params,
                    table.concat(params, '/')))
            else
...
function Controls:get_visibility()
    return Elements:v('speed', 'dragging') and 1 or Elements:maybe('timeline', 'get_is_hovered') and -1 or
    ...
    for c, control in ipairs(self.layout) do
        if not control.hide then
            ...
            elseif sizing == 'dynamic' then
                height = size * scale
                width = max_dynamics_width < width_for_dynamics and height * ratio or width_for_dynamics *
                            ((scale * ratio) / dynamic_units)
            end
