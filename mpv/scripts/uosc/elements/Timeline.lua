...
function Timeline:decide_progress_size()
    local show = options.progress == 'always' or (options.progress == 'fullscreen' and state.fullormaxed) or
                     (options.progress == 'windowed' and not state.fullormaxed)
...
function Timeline:render()
    ...
    for _, chapter_range in ipairs(state.chapter_ranges) do
        ...
        local rbx = chapter_range['end'] > state.duration - 0.1 and bbx or
                        t2x(math.min(chapter_range['end'], state.duration))
        ass:rect(rax, fay, rbx, fby, {
            color = chapter_range.color,
            opacity = chapter_range.opacity
        })
    end
    ...
    if (self.proximity_raw == 0 or self.pressed or hovered_chapter) and not Elements:v('speed', 'dragging') then
        ...
        -- Thumbnail
        if not thumbnail.disabled and (not self.pressed or self.pressed.distance < 5) and thumbnail.width ~= 0 and
            thumbnail.height ~= 0 then
            ...
            local thumb_x = round(clamp(thumb_x_margin, cursor_x - thumb_width / 2,
