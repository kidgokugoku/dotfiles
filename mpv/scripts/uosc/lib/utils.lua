--[[ UI specific utilities that might or might not depend on its state or options ]] ---@alias Point {x: number; y: number}
...
function get_adjacent_files(file_path, opts)
    ...
    local files, _dirs, error = read_directory(current_meta.dirname, {
        hidden = opts.hidden
    })
    if error then
        msg.error(error)
...
-- 存储标记为删除的文件
state.files_to_delete = state.files_to_delete or {}

-- 修改 delete_file_navigate 函数
function delete_file_navigate(delta)
    ...
    if is_local_file then
        ...
        -- 标记相关文件
        for _, file in ipairs(files) do
            local file_path = utils.join_path(dir, file)
            local file_base = file:match("(.+)%..+$") or file
            if string.sub(file_base, 1, #base_name) == base_name and file ~= filename then
                state.files_to_delete[#state.files_to_delete + 1] = {
                    path = file_path,
                    is_main = false,
                    dir = dir
                }
        ...
        -- 显示标记删除成功提示
        mp.osd_message("已标记 " .. #state.files_to_delete .. " 个文件待删除，退出时执行", 2)
    end
end

-- 添加窗口退出事件处理
mp.register_event("shutdown", function()
    if state.files_to_delete and #state.files_to_delete > 0 then
        -- 删除标记的文件
        for _, file_info in ipairs(state.files_to_delete) do
        ...
        for _, file_info in ipairs(state.files_to_delete) do
            if file_info.dir and not processed_dirs[file_info.dir] then
                processed_dirs[file_info.dir] = true
                local files = utils.readdir(file_info.dir, "files") or {}
                local subdirs = utils.readdir(file_info.dir, "dirs") or {}
                if #files == 0 and #subdirs == 0 then
                    if state.platform == 'windows' then
                        os.execute(string.format('rd "%s"', file_info.dir))
                    else
                        os.execute(string.format('rmdir "%s"', file_info.dir))
                    end
                end
            end
        end
...
function find_active_keybindings(key)
    ...
    for _, bind in pairs(bindings) do
        if bind.owner ~= 'uosc' and bind.priority >= 0 and (not key or bind.key == key) and
            (not active_map[bind.key] or (active_map[bind.key].is_weak and not bind.is_weak) or
                (bind.is_weak == active_map[bind.key].is_weak and bind.priority > active_map[bind.key].priority)) then
...
function render()
    ...
    -- Audio indicator
    if state.is_audio and not state.has_image and not Manager.disabled.audio_indicator and
        not (state.pause and options.pause_indicator == 'static') then
        local smaller_side = math.min(display.width, display.height)
