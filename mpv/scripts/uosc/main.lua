--[[ uosc | https://github.com/tomasklaen/uosc ]] local uosc_version = '5.6.0'
...
function update_config()
    ...
    -- Opacity
    config.opacity = table_assign({}, config_defaults.opacity,
...
function create_default_menu_items()
    return { -- {title = t('Subtitles'), value = 'script-binding uosc/subtitles'},
    ...
    -- {title = t('Chapters'), value = 'script-binding uosc/chapters'},
    {
        ...
        -- {title = t('Key bindings'), value = 'script-binding uosc/keybinds'},
        {
            ...
            title = t('Update uosc'),
            value = 'script-binding uosc/update'
        }}
    }, {
        title = t('Delete file & Next'),
        value = 'script-binding uosc/delete-file-next'
...
function update_duration()
    local duration = state._duration and
                         ((state.rebase_start_time == false and state.start_time) and
                             (state._duration + state.start_time) or state._duration)
...
function update_margins()
    ...
    if top == state.margin_top and bottom == state.margin_bottom and left == state.margin_left and right ==
        state.margin_right then
        return
    end
...
bind_command('playlist', create_self_updating_menu_opener({
    ...
    on_remove = function(event)
        mp.commandv('playlist-remove', tostring(event.index - 1))
    end
}))
